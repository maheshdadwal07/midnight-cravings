...
export default function Admin() {
  ...
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState(null);
  const [deleteType, setDeleteType] = useState("product"); // product/user/seller
  ...
  const handleCreateProduct = async () => {
    ...
    try {
      ...
      Object.entries(form).forEach(([k, v]) => {
        if (!v) return;
        if (k === "imageFile") formData.append("image", v);
        else formData.append(k, v);
      });
  ...
  const handleToggleBan = async (id) => {
    try {
      ...
      setSellers((prev) =>
        prev.map((s) =>
          s._id === id ? { ...s, banned: res.data.banned } : s
        )
      );
    } catch {
  ...
  const renderUsers = () => (
    <div>
      ...
      {users.length === 0 ? (
        ...
        <ul className="list">
          {users.map((u) => (
            <li key={u._id} className="item flexBetween">
              <div>
                <div className="bold">{u.name}</div>
                <div className="smallText">{u.email}</div>
              </div>
              <button
  ...
  const renderSellers = () => (
    <div>
      <h3 className="title">All Sellers</h3>
      {sellers.length === 0 ? (
        <p>No sellers found.</p>
      ) : (
        <ul className="list">
          {sellers.map((s) => (
            <li key={s._id} className="item flexBetween">
              <div>
                <div className="bold">{s.name}</div>
                <div className="smallText">{s.email}</div>
                {s.banned && <div className="textRed smallText">Banned</div>}
              </div>
              <div className="actions">
                <button
                  className={`btn ${s.banned ? "btnGreen" : "btnGray"}`}
                  onClick={() => handleToggleBan(s._id)}
                >
                  {s.banned ? "Unban" : "Ban"}
                </button>
                <button
  ...
  const renderProducts = () => (
    <div>
      ...
      <ul className="list">
        {products.map((p) => (
          <li key={p._id} className="item flexBetween">
            <div>
        ...
        <ul className="list">
          {orders.map((o) => (
            <li key={o._id} className="item">
              ...
              <div className="smallText">Status: {o.status}</div>
            </li>
          ))}
        </ul>
      )}
  ...
  return (
    <div className="container">
      <h2 className="mainTitle">Admin Dashboard</h2>
      <p className="subtitle">
        Manage users, sellers, products, and view orders.
      </p>
      ...
      >
        ...
        <div className="actionsRight">
          ...
          <button className="btn btnRed" onClick={handleDelete}>
            Delete
          </button>
        </div>
      </Modal>
      ...
      <style>{`
        ...
        .tabBtn { padding: 8px 16px; border: 1px solid #e5e7eb; background: #f9fafb; border-radius: 8px; cursor: pointer; }
        .tabBtn.active { background: #6366f1; color: white; border: none; }
        .card { background: #fff; border-radius: 10px; padding: 18px; box-shadow: 0 6px 18px rgba(15,23,42,0.06); }
        .list { list-style: none; padding: 0; margin-top: 8px; }
        .item { padding: 10px; border-bottom: 1px solid #f3f4f6; }
        .bold { font-weight: 600; }
        .textGray { color: #6b7280; }
        .textRed { color: #ef4444; }
        .smallText { font-size: 13px; color: #9ca3af; }
        .btn { padding: 6px 12px; border-radius: 6px; cursor: pointer; font-size: 14px; }
        .btnGray { background: #f3f4f6; border: 1px solid #e5e7eb; }
        .btnBlue { background: #6366f1; color: white; border: none; }
        .btnRed { background: #ef4444; color: white; border: none; }
        .btnGreen { background: #10b981; color: white; border: none; }
        .actions { display: flex; gap: 8px; }
        .actionsRight { display: flex; justify-content: flex-end; gap: 8px; margin-top: 12px; }
        .flexBetween { display: flex; justify-content: space-between; align-items: center; }
        .input { padding: 8px; border-radius: 6px; border: 1px solid #e5e7eb; }
        .full { width: 100%; margin-top: 8px; padding: 10px; }
        .formGrid { display: grid; gap: 8px; }
